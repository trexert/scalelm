...
async fn setup_channel(connection: &Connection) -> anyhow::Result<Channel> {
    ...
    let rabbitmq_my_response_queue = env::var("RABBITMQ_MY_RESPONSE_QUEUE")?;
    let channel = connection.open_channel(None).await?;

    let (response_queue_name, _, _) = channel
        .queue_declare(QueueDeclareArguments::new(&rabbitmq_my_response_queue))
        .await?
        .unwrap();

    channel
        .queue_bind(QueueBindArguments::new(&response_queue_name, "", ""))
        .await?;
